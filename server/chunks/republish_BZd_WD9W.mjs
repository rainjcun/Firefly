...
async function POST({ request }) {
  try {
    ...
    const description = $2('meta[name="description"]').attr("content") || $2('meta[property="og:description"]').attr("content") || $2("p").first().text().substring(0, 150).trim();
    let content = "";
    ...
    if (!foundContent) {
      const bodyContent = $2("body").not("script").not("style").not("header").not("footer").not("nav").html() || "";
      content = bodyContent;
    }
    const contentCleaned = content.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "").replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "").replace(/<iframe[^>]*>[\s\S]*?<\/iframe>/gi, "").replace(/<noscript[^>]*>[\s\S]*?<\/noscript>/gi, "").replace(/<form[^>]*>[\s\S]*?<\/form>/gi, "").replace(/<svg[^>]*>[\s\S]*?<\/svg>/gi, "").replace(/<!--[\s\S]*?-->/g, "").replace(/\s+/g, " ").trim();
